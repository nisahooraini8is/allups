...
def jobOders():
    ...
    # Preserve the show_archived parameter when paginating
    if show_archived:
        job_orders = Joborder.query.filter(Joborder.archived == True)
    else:
        job_orders = Joborder.query.filter(Joborder.archived == False)

    job_orders = job_orders.order_by(desc(Joborder.id)).paginate(page=page, per_page=per_page)

    total_pages = job_orders.pages
    start_page = max(1, page - 5)
    end_page = min(total_pages, start_page + 10)

    marketing_entries = db.session.query(Marketing).all()
    company_name_dict = {entry.id: entry.company for entry in marketing_entries}
    for job_order in job_orders.items:
        company_id = job_order.company_id
        if company_id in company_name_dict:
            job_order.company = company_name_dict[company_id]
    ...
    # Preserve the show_archived parameter in pagination links
    pagination_links = {
        'show_archived': show_archived
    }

    return render_template('jobsorder.html', jobsorder=job_orders, alldata=alldata, page=page, total_pages=total_pages,
                           start_page=start_page, end_page=end_page, pagination_links=pagination_links)
...
def Companies():
    ...
    per_page = 35
    unique_companies = {}
    all_companies = Marketing.query.order_by(desc(Marketing.id)).all()
...
def position():
    ...
    positions = Joborder.query.filter(
        and_(Joborder.company_id == selectedOption, Joborder.jobstatus == 'active')).all()
